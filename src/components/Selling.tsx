...
// ${link.bgColor !== link.bgColor ? link.bgColor : "bg-white" }
export function Selling() {
    return (
        <div className="flex flex-col gap-8 xl:flex-row xl:gap-16">
            {selling.map((link) => (
                <div key={link.id} className={`flex flex-col justify-center items-center gap-8 px-10 py-10 rounded-xl bg-${link.bgColor}`}>
